    ...
    def get_url(self, url: str, referer: str = None,
        ...
        try:
            response = requests.get(
                url, 
                headers=headers, 
                headers={"Referer": "https://disk.yandex.com.tr/"}
            )
...
class DzenRu(ExtractorAPI):
    ...
    def get_url(self, url: str, referer: str = None,
        ...
        try:
            response = requests.get(url, headers=headers, headers={"Referer": self.main_url})
            if response.status_code != 200:
                return
...
def create_m3u(links: List[ExtractorLink], filename: str = "playlist.m3u"):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("#EXTM3U\n")
        f.write("#EXTINF:-1 tvg-logo=\"\" group-title=\"İnatBox\", İnatBox Playlist\n")
...
def main():
    ...
    if categories:
        ...
        if category_url:
            print(f"\n=== Kategori Detayları: {category_url} ===")
            ...
            if data:
                ...
                if results:
                    ...
                    print(f"\n=== İlk içerik: {first_result['name']} ===")
                    
                    # İçeriği yükle
                    content = inat.load(first_result["url"])
                    if content:
                        ...
                        if content.get('type') == 'Live':
                            ...
                            if ch_content:
                                ...
                                for link in links:
                                    ...
                                    print(f"  Tip: {link.type}, Kalite: {link.quality}")
                                
                                # M3U oluştur
                                if links:
                                    create_m3u(links, "inatbox_playlist.m3u")
                                    print("\nM3U dosyası oluşturuldu: inatbox_playlist.m3u")
                        elif content.get('type') == 'TvSeries':
                            episodes = content.get('episodes', [])
                            if episodes:
                                first_episode = episodes[0]
                                print(f"\n=== İlk bölüm: {first_episode['name']} ===")
